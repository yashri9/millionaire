        ...
        return (
          <button
            ...
            className={`flex flex-col border p-6 text-left transition-colors md:p-8 ${
              on ? "border-foreground bg-accent/25" : "border-border bg-background hover:bg-muted"
            }`}
          >
            <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-md border border-border bg-background">
              <div
                className={`waveform ${on ? "text-foreground" : "text-muted-foreground"}`}
                aria-hidden
              >
                <span />
                <span />
            ...
            <div className="mb-3 flex items-start justify-between gap-3">
              <h3 className="text-xl font-bold md:text-2xl">{v.name}</h3>
              <span className="text-sm text-muted-foreground">{v.meta}</span>
            </div>
            <p className="text-muted-foreground">{v.desc}</p>
            <div className="mt-6 text-sm font-semibold">▶ Preview voice</div>
          </button>
        );
      })}
    </div>
