...
export async function uploadAndProcessPdf(
  ...
  if (!prepared.ok || !prepared.data.signedUrl || !prepared.data.deck?.id) {
    throw new Error(prepared.data.error || "Could not start upload. Sign in and try again.");
  }
  const deckId = prepared.data.deck.id;
  ...
  if (!put.ok) {
    throw new Error("Could not upload the PDF. Please try again.");
  }
  onProgress?.({ phase: "upload", current: 1, total: 1 });
  onProgress?.({ phase: "parse", current: 0, total: 1 });

  const parsed = await fetchJson<{ error?: string; warning?: string }>(
    `/api/decks/${deckId}/parse`,
    { method: "POST" },
  );
  if (!parsed.ok) {
    throw new Error(parsed.data.error || "We couldn't process this PDF. Please try again.");
  }
